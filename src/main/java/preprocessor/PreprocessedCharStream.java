...
public class PreprocessedCharStream extends UnbufferedCharStream {
	protected List<PreprocessedToken> tokens;
	...
	public PreprocessedCharStream(List<PreprocessedToken> tokens) {
		this.tokens = tokens;
	...
	protected int nextChar() throws IOException {
		...
		if ( c == text.length() ) {
			...
			if ( tp==tokens.size() ) return CharStream.EOF;
			c = 0;
			PreprocessedToken t = tokens.get(tp);
			text = t.getText();
	...
	public int getLineFromCharIndex(int ci) {
		int ti = getTokenIndexFromCharIndex(ci);
		if ( ti==-1 ) return -1;
		PreprocessedToken t = tokens.get(ti);
	...
	protected void computeTokenCharRanges(List<PreprocessedToken> tokens) {
		tokenCharIntervals = new ArrayList<Interval>();
		int absCharIndex = 0;
		for (PreprocessedToken t : tokens) {
	...
	protected void computeTokenLineRanges(List<PreprocessedToken> tokens) {
		int absCharIndex = 0;
		for (PreprocessedToken t : tokens) {
